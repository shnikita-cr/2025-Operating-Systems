...
static bool
recvFrame(ConnIface &c, unsigned char &out_type, std::vector<unsigned char> &out_payload, int timeoutMs) {
    unsigned char hdr[9];
    ...
    if (len > 0) {
        if (!c.readExact(out_payload.data(), len, timeoutMs)) return false;
    }
    return true;
}

static std::string semH2C(int hostPid, int clientId) {
    return makeSemName("h2c", hostPid, clientId, "mq");
}

static std::string semC2H(int host_pid, int clientId) {
    return makeSemName("c2h", host_pid, clientId, "mq");
}
...
class MqClientLink final : public IClientLink {
public:
    MqClientLink(int host_pid,
                 int client_id,
                 const std::string &nick,
                 std::unique_ptr<ConnMq> conn,
                 NamedSemaphore sem_h2c,
                 NamedSemaphore sem_c2h,
                 const std::string &q_c2h,
                 const std::string &q_h2c,
                 std::function<void(int)> on_disconnect)
            : host_pid_(host_pid),
              client_id_(client_id),
              nick_(nick),
              conn_(std::move(conn)),
              sem_h2c_(std::move(sem_h2c)),
              sem_c2h_(std::move(sem_c2h)),
              q_c2h_(q_c2h),
              q_h2c_(q_h2c),
              on_disconnect_(std::move(on_disconnect)) {}

    ~MqClientLink() override { disconnect(); }
    ...
    bool requestTurn(const TurnRequest &req, int timeoutMs, TurnResponse &out) override {
        ...
        if (!sendFrame(*conn_, MQ_TURN_REQ, p, tmo)) {
    ...
    bool sendResult(const ResultMessage &msg, int timeoutMs) override {
        ...
        if (!sendFrame(*conn_, MQ_RESULT, p, tmo)) {
    ...
    void disconnect() override {
        ...
        if (!q_c2h_.empty()) ConnMq::unlinkQuiet(q_c2h_);
        if (!q_h2c_.empty()) ConnMq::unlinkQuiet(q_h2c_);
...
class MqHostSession final : public IHostSession {
public:
    MqHostSession(const HostArgs &args, const std::shared_ptr<Logger> &log)
            : args_(args), log_(log), host_pid_(getPid()) {}

    ~MqHostSession() override { shutdown(); }
    ...
    void startAccepting() override {
        ...
        if (log_) log_->info("MqHostSession started. Waiting for clients via SIGUSR1...");
    }
    ...
    void shutdown() override {
        ...
        std::unordered_map<int, std::shared_ptr<MqClientLink>> local;
        {
            std::lock_guard<std::mutex> lk(mu_);
            local.swap(clients_);
        }
        ...
        if (log_) log_->info("MqHostSession shutdown complete");
    }
...
private:
    void HandshakeLoop() {
        while (!stop_.load()) {
            ...
            std::string q_c2h, q_h2c;
            ConnMq conn;
            if (!ConnMq::hostCreateAndOpen(host_pid_, ev.clientId, conn, q_c2h, q_h2c, e)) {
                if (log_) log_->error("MQ create/open failed: " + e);
                continue;
            }
            ...
            if (!sem_h2c.isValid()) {
                ...
                conn.close();
                ConnMq::unlinkQuiet(q_c2h);
                ConnMq::unlinkQuiet(q_h2c);
                continue;
            }
            ...
            std::unique_ptr<ConnMq> connp(new ConnMq(std::move(conn)));
...

            {
                ...
                std::shared_ptr<MqClientLink> link(new MqClientLink(
                ...
                if (log_) {
                    log_->info("Client connected (mq): id=" + std::to_string(ev.clientId) +
...
private:
    ...
    mutable std::mutex mu_;
    std::unordered_map<int, std::shared_ptr<MqClientLink>> clients_;
...
class MqClientSession final : public IClientSession {
public:
    MqClientSession(const ClientArgs &args, const std::shared_ptr<Logger> &log)
            : args_(args), log_(log) {}

    ~MqClientSession() override { shutdown(); }
    ...
    bool connect() override {
        ...
        ConnMq conn;
        std::string q_c2h, q_h2c;
        if (!ConnMq::clientOpenWithRetry(args_.hostPid, client_id_, tmo, conn, q_c2h, q_h2c, e)) {
            last_error_ = "MQ open failed: " + e;
            if (log_) log_->error(last_error_);
            return false;
        }
        conn_.reset(new ConnMq(std::move(conn)));
        ...
        if (!sendFrame(*conn_, MQ_HELLO, p, tmo)) {
        ...
        if (log_) {
            log_->info("Connected (mq). clientId=" + std::to_string(client_id_) +
                       " q_c2h=" + q_c2h +
                       " q_h2c=" + q_h2c);
        }
    ...
    bool waitTurnRequest(int timeout_ms, TurnRequest &outReq) override {
        if (!connected_) {
    ...
    bool sendTurn(const TurnResponse &resp, int timeoutMs) override {
        ...
        if (!sendFrame(*conn_, MQ_TURN_RESP, p, tmo)) {
...
HostRuntime createHostRuntime(int argc, char **argv) {
    ...
    if (!parseHostArgs(argc, argv, rt.args, &err)) {
        std::fprintf(stderr, "host_mq args error: %s\n", err.c_str());
        return rt;
    }

    rt.log = std::make_shared<Logger>(rt.args.logDir, rt.transportCode, "host", rt.args.nick, "host.log");
    rt.session = std::unique_ptr<IHostSession>(new MqHostSession(rt.args, rt.log));
    return rt;
}
...
ClientRuntime createClientRuntime(int argc, char **argv) {
    ...
    if (!parseClientArgs(argc, argv, rt.args, &err)) {
        std::fprintf(stderr, "client_mq args error: %s\n", err.c_str());
        return rt;
    }
    ...
    rt.session = std::unique_ptr<IClientSession>(new MqClientSession(rt.args, rt.log));
    return rt;
}
