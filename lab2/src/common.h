...
static_assert(sizeof(Msg) == 52, "Msg must be 52 bytes");
...
class Logger {
public:
    explicit Logger(const std::string& path, const char* who);
    ~Logger();
...
// stdin number with 3s timeout; returns -1 if none/invalid
...
// Write host pid to /app/run/host.pid (best-effort)
